...
import com.quanta.aj.yunanjian.mywegit.Dialog_addEvidenceActivity;
...
import com.quanta.aj.yunanjian.page.enforcement.EfEvidencePage;
...
public class Document09Activity extends MFragmentActivity {
    private static final String TAG = "Document09Activity";

    public static Intent getIntent(Context ctx, EfDocument efDocument,Boolean isDone) {
        Intent intent = new Intent(ctx, Document09Activity.class);
    ...
    @ViewInject(R.id.cywplist)
    private ListView cywplist;
    @ViewInject(R.id.no)
    private EditText no;
    ...
    private EfDocument09 document09 = new EfDocument09();
    ...
    private EfEvidencePage efEvidencePage = new EfEvidencePage();
    private List evidenceitme = new ArrayList();
    private List<EfEvidence> efEvidences = new LinkedList<>();
    private CorpInfo corpInfo;

    @Override
    protected void onCreate(Bundle savedInstanceState) {
        super.onCreate(savedInstanceState);
        setContentView(R.layout.activity_document09);
        x.view().inject(this);
        mContext = Document09Activity.this;
        ...
        domtt.setTitle2(DocumentType.d9.name);
    ...
    private void getData() {
        if (null != efdcoment){
            new Thread(new Runnable() {
                @Override
                public void run() {
                    RequestParams params = new RequestParams(Constant.getHOST(mContext)+URLs.DOCUMENT_URL+"/09/get.do");
                    params.addBodyParameter("id",efdcoment.getDocument());
                    x.http().post(params, new Callback.CommonCallback<String>() {
                        @Override
                        public void onSuccess(String s) {
                            document09 = getGson().fromJson(s,EfDocument09.class);
    ...
    private void initData() {
        no.setText(document09.getNo());
        corp.setText(document09.getCorp());
        name.setText(document09.getName());
        phone.setText(document09.getPhone());
        corpAddress.setText(document09.getCorpAddress());
        yzbm.setText(document09.getYzbm());
        address.setText(document09.getAddress());
        startTime.setText(getFmt().format(document09.getStartTime()));
        endTime.setText(getFmt().format(document09.getEndTime()));
    }
    private void getZfry() {
        if (null!=document09.getId()){
            //获取执法人员
            new Thread(new Runnable() {
                @Override
                public void run() {
                    RequestParams params = new RequestParams(Constant.getHOST(mContext)+URLs.GETZFRY_URL);
                    efUserpage.setDocument(document09.getId());
                    params.addParameter("page",efUserpage);
                    params.addBodyParameter("document",document09.getId());
                    x.http().post(params, new Callback.CommonCallback<String>() {
                        @Override
                        public void onSuccess(String s) {
    ...
    private void toComit(final Boolean submit) {
        if (efUsers == null||efUsers.size()<1){
            ...
            document09.setNo(no.getText().toString());
            document09.setCorp(corp.getText().toString());
            document09.setName(name.getText().toString());
            document09.setPhone(phone.getText().toString());
            document09.setCorpAddress(corpAddress.getText().toString());
            document09.setYzbm(yzbm.getText().toString());
            document09.setAddress(address.getText().toString());
            try {
                document09.setStartTime(getFmt().parse(startTime.getText().toString()));
                document09.setEndTime(getFmt().parse(endTime.getText().toString()));
            ...
            new Thread(new Runnable() {
                ...
                public void run() {
                    ...
                    form.setEfEvidences(efEvidences);
                    RequestParams params = new RequestParams(Constant.getHOST(mContext)+URLs.SAVEDOM_URL + "/09/appsave.do");
    ...
    private void getEvidence(){
        if (null!=document09.getId()){
            new Thread(new Runnable() {
                @Override
                public void run() {
                    RequestParams params = new RequestParams(Constant.getHOST(mContext)+URLs.PEGEEVIDENCE_URL);
                    efEvidencePage.setDocument(document09.getId());
                    params.addParameter("page",efEvidencePage);
                    params.addBodyParameter("document",document09.getId());
                    x.http().post(params, new Callback.CommonCallback<String>() {
                        @Override
                        public void onSuccess(String s) {
                            EfEvidencePage efEvidencePage = JsonUtils.parseObject(s,EfEvidencePage.class);
                            efEvidences = efEvidencePage.getRows();
    ...
    private void addEvidence(List<EfEvidence> efEvidences){
        myListAdapter = new MyListAdapter<EfEvidence>(efEvidences,R.layout.item_evidence) {
            @Override
            public void bindView(final ViewHolder holder, EfEvidence obj) {
                holder.setText(R.id.name,obj.getName());
                holder.setText(R.id.type,obj.getType());
                holder.setText(R.id.number,obj.getNumber());
                holder.setText(R.id.remark,obj.getRemark());
                holder.setOnClickListener(R.id.check, new View.OnClickListener() {
                    @Override
                    public void onClick(View v) {
                        CheckBox box = (CheckBox)v;
                        if (box.isChecked()){
                            evidenceitme.add(holder.getItemPosition()); //获得选中的列传出值去
                        }
                    }
                });
            }
        };
        cywplist.setAdapter(myListAdapter);
    }
    private void delEvidence(){
        if (null!=evidenceitme){
            for (int i=evidenceitme.size()-1;i>=0;i--){
                efEvidences.remove(Integer.parseInt(String.valueOf(evidenceitme.get(i))));
            }
            evidenceitme.clear();//清空以备下次使用
            addEvidence(efEvidences);
        }
    }
    @Override
        ...
        switch (requestCode){
            ...
            case 2:
                if (resultCode == RESULT_OK){
                    ...
                    EfEvidence efEvidence = new EfEvidence();
                    efEvidence.setName(name);
                    efEvidence.setType(type);
                    efEvidence.setNumber(number);
                    efEvidence.setRemark(remark);
                    efEvidences.add(efEvidence);
                    addEvidence(efEvidences);
                }
                break;
            case 3:
    ...
    @Event(value = {R.id.addzfry,R.id.delzfry,R.id.comit,R.id.print,R.id.startTime,R.id.endTime,R.id.addwp,R.id.delwp,R.id.comit2})
    private void clink(View v){
        switch (v.getId()){
            ...
            case R.id.endTime:
                ...
                break;
            case R.id.addwp:
                startActivityForResult(Dialog_addEvidenceActivity.getIntent(mContext,Dialog_addEvidenceActivity.class),2);
                break;
            case R.id.delwp:
                delEvidence();
                break;
