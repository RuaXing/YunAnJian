...
public class Document10Activity extends MFragmentActivity {
    private static final String TAG = "Document10Activity";

    public static Intent getIntent(Context ctx, EfDocument efDocument,Boolean isDone) {
        Intent intent = new Intent(ctx, Document10Activity.class);
    ...
    private EfDocument10 document10 = new EfDocument10();
    ...
    protected void onCreate(Bundle savedInstanceState) {
        ...
        setContentView(R.layout.activity_document10);
        x.view().inject(this);
        mContext = Document10Activity.this;
        ...
        domtt.setTitle2(DocumentType.d10.name);
    ...
    private void getData() {
        if (null != efdcoment){
            new Thread(new Runnable() {
                @Override
                public void run() {
                    RequestParams params = new RequestParams(Constant.getHOST(mContext)+URLs.DOCUMENT_URL+"/10/get.do");
                    params.addBodyParameter("id",efdcoment.getDocument());
                    x.http().post(params, new Callback.CommonCallback<String>() {
                        @Override
                        public void onSuccess(String s) {
                            document10 = getGson().fromJson(s,EfDocument10.class);
    ...
    private void initData() {
        no.setText(document10.getNo());
        fromNo.setText(document10.getFromNo());
        endTime.setText(getFmt().format(document10.getEndTime()));
        corp.setText(document10.getCorp());
        requirement.setText(document10.getRequirement());
    }
    private void toComit(final Boolean submit) {
        document10.setNo(no.getText().toString());
        document10.setFromNo(fromNo.getText().toString());
        try {
            document10.setEndTime(getFmt().parse(endTime.getText().toString()));
        } catch (ParseException e) {
            e.printStackTrace();
        }
        document10.setCorp(corp.getText().toString());
        document10.setRequirement(requirement.getText().toString());
        new Thread(new Runnable() {
            @Override
            public void run() {
                form.setDocument10(document10);
                ...
                RequestParams params = new RequestParams(Constant.getHOST(mContext)+URLs.SAVEDOM_URL+"/10/appsave.do");
    ...
    @Event(value = {R.id.comit,R.id.print,R.id.endTime,R.id.comit2})
