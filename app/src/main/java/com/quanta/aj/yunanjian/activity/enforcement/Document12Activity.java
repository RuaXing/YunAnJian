...
public class Document12Activity extends MFragmentActivity {
    private static final String TAG = "Document12Activity";

    public static Intent getIntent(Context ctx, EfDocument efDocument,Boolean isDone) {
        Intent intent = new Intent(ctx, Document12Activity.class);
    ...
    @ViewInject(R.id.dcrq)
    private TextView dcrq;
    @ViewInject(R.id.fs)
    private EditText fs;
    @ViewInject(R.id.ys)
    private EditText ys;
    @ViewInject(R.id.fxwt)
    private EditText fxwt;
    @ViewInject(R.id.flyj)
    private EditText flyj;
    @ViewInject(R.id.fscl)
    private EditText fscl;


    private Context mContext;
    private EfDocument efdcoment;
    private EfDocument12 document12 = new EfDocument12();
    ...
    protected void onCreate(Bundle savedInstanceState) {
        ...
        setContentView(R.layout.activity_docuemnt12);
        x.view().inject(this);
        mContext = Document12Activity.this;
        ...
        domtt.setTitle2(DocumentType.d12.name);
    ...
    private void getData() {
        if (null != efdcoment){
            new Thread(new Runnable() {
                @Override
                public void run() {
                    RequestParams params = new RequestParams(Constant.getHOST(mContext)+URLs.DOCUMENT_URL+"/12/get.do");
                    params.addBodyParameter("id",efdcoment.getDocument());
                    x.http().post(params, new Callback.CommonCallback<String>() {
                        @Override
                        public void onSuccess(String s) {
                            document12 = getGson().fromJson(s,EfDocument12.class);
    ...
    private void initData() {
        no.setText(document12.getNo());
        sysjg.setText(document12.getSysjg());
        dcrq.setText(getFmt().format(document12.getDcrq()));
        fs.setText(document12.getFs());
        ys.setText(document12.getYs());
        fxwt.setText(document12.getFxwt());
        flyj.setText(document12.getFlyj());
        fscl.setText(document12.getFscl());
    }
    private void toComit(final Boolean submit) {
        document12.setNo(no.getText().toString());
        document12.setSysjg(sysjg.getText().toString());
        try {
            document12.setDcrq(getFmt().parse(dcrq.getText().toString()));
        } catch (ParseException e) {
            e.printStackTrace();
        }
        document12.setFs(fs.getText().toString());
        document12.setYs(ys.getText().toString());
        document12.setFxwt(fxwt.getText().toString());
        document12.setFscl(fscl.getText().toString());
        new Thread(new Runnable() {
            @Override
            public void run() {
                form.setDocument12(document12);
                ...
                RequestParams params = new RequestParams(Constant.getHOST(mContext)+URLs.SAVEDOM_URL+"/12/appsave.do");
    ...
    @Event(value = {R.id.comit,R.id.print,R.id.dcrq,R.id.comit2})
    private void clink(View v){
        switch (v.getId()){
            ...
            case R.id.print:
                ...
                break;
            case R.id.dcrq:
                SlideDateTimeListener dcrqlistener = new SlideDateTimeListener() {
                    @Override
                    public void onDateTimeSet(Date date) {
                        dcrq.setText(getFmt().format(date));
                    }
                };
                addTime(dcrqlistener);
                break;
            case R.id.comit2:
