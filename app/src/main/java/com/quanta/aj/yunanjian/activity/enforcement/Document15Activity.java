...
public class Document15Activity extends MFragmentActivity {
    private static final String TAG = "Document15Activity";

    public static Intent getIntent(Context ctx, EfDocument efDocument,Boolean isDone) {
        Intent intent = new Intent(ctx, Document15Activity.class);
    ...
    private EfDocument15 document15 = new EfDocument15();
    ...
    protected void onCreate(Bundle savedInstanceState) {
        ...
        setContentView(R.layout.activity_document15);
        x.view().inject(this);
        mContext = Document15Activity.this;
        ...
        domtt.setTitle2(DocumentType.d15.name);
    ...
    private void getData() {
        if (null != efdcoment){
            new Thread(new Runnable() {
                @Override
                public void run() {
                    RequestParams params = new RequestParams(Constant.getHOST(mContext)+URLs.DOCUMENT_URL+"/15/get.do");
                    params.addBodyParameter("id",efdcoment.getDocument());
                    x.http().post(params, new Callback.CommonCallback<String>() {
                        @Override
                        public void onSuccess(String s) {
                            document15 = getGson().fromJson(s,EfDocument15.class);
    ...
    private void initData() {
        personName.setText(document15.getPersonName());
        caseName.setText(document15.getCaseName());
        dsrjbqk.setText(document15.getDsrjbqk());
        ajjbqk.setText(document15.getAjjbqk());
        evidenceName.setText(document15.getEvidenceName());
        evidenceTimes.setText(document15.getEvidenceTimes());
        tqly.setText(document15.getTqyj());
        tqyj.setText(document15.getTqly());
        zjbcfs.setText(document15.getZjbcfs());
    }
    private void toComit(final Boolean submit) {
        document15.setPersonName(personName.getText().toString());
        document15.setCaseName(caseName.getText().toString());
        document15.setDsrjbqk(dsrjbqk.getText().toString());
        document15.setAjjbqk(ajjbqk.getText().toString());
        document15.setEvidenceName(evidenceName.getText().toString());
        document15.setEvidenceTimes(evidenceTimes.getText().toString());
        document15.setTqly(tqly.getText().toString());
        document15.setTqyj(tqyj.getText().toString());
        document15.setZjbcfs(zjbcfs.getText().toString());
        new Thread(new Runnable() {
            @Override
            public void run() {
                form.setDocument15(document15);
                ...
                RequestParams params = new RequestParams(Constant.getHOST(mContext)+URLs.SAVEDOM_URL+"/15/appsave.do");
