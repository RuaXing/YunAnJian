...
public class Document16Activity extends MFragmentActivity {
    private static final String TAG = "Document16Activity";

    public static Intent getIntent(Context ctx, EfDocument efDocument,Boolean isDone) {
        Intent intent = new Intent(ctx, Document16Activity.class);
    ...
    @ViewInject(R.id.decide)
    private EditText decide;

    private Context mContext;
    private EfDocument efdcoment;
    private EfDocument16 document16 = new EfDocument16();
    ...
    protected void onCreate(Bundle savedInstanceState) {
        ...
        setContentView(R.layout.activity_document16);
        x.view().inject(this);
        mContext = Document16Activity.this;
        ...
        domtt.setTitle2(DocumentType.d16.name);
    ...
    private void getData() {
        if (null != efdcoment){
            new Thread(new Runnable() {
                @Override
                public void run() {
                    RequestParams params = new RequestParams(Constant.getHOST(mContext)+URLs.DOCUMENT_URL+"/16/get.do");
                    params.addBodyParameter("id",efdcoment.getDocument());
                    x.http().post(params, new Callback.CommonCallback<String>() {
                        @Override
                        public void onSuccess(String s) {
                            document16 = getGson().fromJson(s,EfDocument16.class);
    ...
    private void initData() {
        no.setText(document16.getNo());
        corp.setText(document16.getCorp());
        notice.setText(document16.getNotice());
        time.setText(getFmt().format(document16.getTime()));
        goods.setText(document16.getGoods());
        decide.setText(document16.getDecide());
    }
    private void toComit(final Boolean submit) {
        document16.setNo(no.getText().toString());
        document16.setCorp(corp.getText().toString());
        document16.setNotice(notice.getText().toString());
        try {
            document16.setTime(getFmt().parse(time.getText().toString()));
        } catch (ParseException e) {
            e.printStackTrace();
        }
        document16.setGoods(goods.getText().toString());
        document16.setDecide(decide.getText().toString());
        new Thread(new Runnable() {
            @Override
            public void run() {
                form.setDocument16(document16);
                ...
                RequestParams params = new RequestParams(Constant.getHOST(mContext)+URLs.SAVEDOM_URL+"/16/appsave.do");
    ...
    @Event(value = {R.id.comit,R.id.print,R.id.time,R.id.comit2})
