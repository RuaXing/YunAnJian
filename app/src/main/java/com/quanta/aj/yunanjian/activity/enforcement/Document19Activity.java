...
public class Document19Activity extends MFragmentActivity {
    private static final String TAG = "Document19Activity";

    public static Intent getIntent(Context ctx, EfDocument efDocument,Boolean isDone) {
        Intent intent = new Intent(ctx, Document19Activity.class);
    ...
    @ViewInject(R.id.tzhjbqk)
    private EditText tzhjbqk;

    private Context mContext;
    private EfDocument efdcoment;
    private EfDocument19 document19 = new EfDocument19();
    ...
    protected void onCreate(Bundle savedInstanceState) {
        ...
        setContentView(R.layout.activity_document19);
        x.view().inject(this);
        mContext = Document19Activity.this;
        ...
        domtt.setTitle2(DocumentType.d19.name);
    ...
    private void getData() {
        if (null != efdcoment){
            new Thread(new Runnable() {
                @Override
                public void run() {
                    RequestParams params = new RequestParams(Constant.getHOST(mContext)+URLs.DOCUMENT_URL+"/19/get.do");
                    params.addBodyParameter("id",efdcoment.getDocument());
                    x.http().post(params, new Callback.CommonCallback<String>() {
                        @Override
                        public void onSuccess(String s) {
                            document19 = getGson().fromJson(s,EfDocument19.class);
    ...
    private void initData() {
        no.setText(document19.getNo());
        ajmc.setText(document19.getAjmc());
        zcr.setText(document19.getZcr());
        tzy.setText(document19.getTzy());
        sjy.setText(document19.getSjy());
        tzhjbqk.setText(document19.getTzhjbqk());
    }
    private void toComit(final Boolean submit) {
        document19.setNo(no.getText().toString());
        document19.setAjmc(ajmc.getText().toString());
        document19.setZcr(zcr.getText().toString());
        document19.setTzy(tzy.getText().toString());
        document19.setSjy(sjy.getText().toString());
        document19.setTzhjbqk(tzhjbqk.getText().toString());
        new Thread(new Runnable() {
            @Override
            public void run() {
                form.setDocument19(document19);
                ...
                RequestParams params = new RequestParams(Constant.getHOST(mContext)+URLs.SAVEDOM_URL+"/19/appsave.do");
