...
public class Document21Activity extends MFragmentActivity {
    private static final String TAG = "Document21Activity";

    public static Intent getIntent(Context ctx, EfDocument efDocument,Boolean isDone) {
        Intent intent = new Intent(ctx, Document21Activity.class);
    ...
    private EfDocument21 document21 = new EfDocument21();
    ...
    protected void onCreate(Bundle savedInstanceState) {
        ...
        setContentView(R.layout.activity_document21);
        x.view().inject(this);
        mContext = Document21Activity.this;
        ...
        domtt.setTitle2(DocumentType.d21.name);
        domtt.setTitleSize(20);
        getData();
    }

    private void initCorpData() {
        bcdw.setText(corpInfo.getQymc());
    ...
    private void getData() {
        if (null != efdcoment){
            new Thread(new Runnable() {
                @Override
                public void run() {
                    RequestParams params = new RequestParams(Constant.getHOST(mContext)+URLs.DOCUMENT_URL+"/21/get.do");
                    params.addBodyParameter("id",efdcoment.getDocument());
                    x.http().post(params, new Callback.CommonCallback<String>() {
                        @Override
                        public void onSuccess(String s) {
                            document21 = getGson().fromJson(s,EfDocument21.class);
    ...
    private void initData() {
        no.setText(document21.getNo());
        bcdw.setText(document21.getBcdw());
        ctjg.setText(document21.getCtjg());
        jdglbmdz.setText(document21.getJdglbmdz());
        lxr.setText(document21.getLxr());
        lxdh.setText(document21.getLxdh());
        yzbm.setText(document21.getYzbm());
        wfxw.setText(document21.getWfxw());
        wfgd.setText(document21.getWfgd());
        flyj.setText(document21.getFlyj());
        xzcf.setText(document21.getXzcf());
    }
    private void toComit(final Boolean submit) {
        document21.setNo(no.getText().toString());
        document21.setBcdw(bcdw.getText().toString());
        document21.setCtjg(ctjg.getText().toString());
        document21.setJdglbmdz(jdglbmdz.getText().toString());
        document21.setLxr(lxr.getText().toString());
        document21.setLxdh(lxdh.getText().toString());
        document21.setYzbm(yzbm.getText().toString());
        document21.setWfxw(wfxw.getText().toString());
        document21.setWfgd(wfgd.getText().toString());
        document21.setFlyj(flyj.getText().toString());
        document21.setXzcf(xzcf.getText().toString());
        new Thread(new Runnable() {
            @Override
            public void run() {
                form.setDocument21(document21);
                ...
                RequestParams params = new RequestParams(Constant.getHOST(mContext)+URLs.SAVEDOM_URL+"/21/appsave.do");
