...
public class Document25Activity extends MFragmentActivity {
    private static final String TAG = "Document25Activity";
    public static Intent getIntent(Context ctx, EfDocument efDocument,Boolean isDone) {
        Intent intent = new Intent(ctx, Document25Activity.class);
    ...
    private EfDocument25 document25 = new EfDocument25();
    ...
    protected void onCreate(Bundle savedInstanceState) {
        ...
        setContentView(R.layout.activity_document25);
        x.view().inject(this);
        mContext = Document25Activity.this;
        ...
        domtt.setTitle2(DocumentType.d25.name);
    ...
    private void getData() {
        if (null != efdcoment){
            new Thread(new Runnable() {
                @Override
                public void run() {
                    RequestParams params = new RequestParams(Constant.getHOST(mContext)+URLs.DOCUMENT_URL+"/25/get.do");
                    params.addBodyParameter("id",efdcoment.getDocument());
                    x.http().post(params, new Callback.CommonCallback<String>() {
                        @Override
                        public void onSuccess(String s) {
                            document25 = getGson().fromJson(s,EfDocument25.class);
    ...
    private void initData() {
        no.setText(document25.getNo());
        notice.setText(document25.getNotice());
        fddbr.setText(document25.getFddbr());
        fddbrzw.setText(document25.getFddbrzw());
        frlxdh.setText(document25.getDsrwtdlr());
        corpName.setText(document25.getCorpName());
        address.setText(document25.getAddress());
        yzbm.setText(document25.getYzbm());
        sfdccf.setText(document25.getSfdccf());
        wfss.setText(document25.getWfss());
        wfgd.setText(document25.getWfgd());
        flyj.setText(document25.getFlyj());
        xzcf.setText(document25.getXzcf());
        money.setText(String.valueOf(document25.getMoney()));
        fkssdw.setText(document25.getFkssdw());
        fksszh.setText(document25.getFksszh());
        dsrwtdlr.setText(document25.getDsrwtdlr());
        fyjgsj.setText(document25.getFyjgsj());
        fyjgzf.setText(document25.getFyjgzf());
        ssfy.setText(document25.getSsfy());
    }
    private void getZfry() {
        if (null!=document25.getId()){
            //获取执法人员
            new Thread(new Runnable() {
                @Override
                public void run() {
                    RequestParams params = new RequestParams(Constant.getHOST(mContext)+URLs.GETZFRY_URL);
                    efUserpage.setDocument(document25.getId());
                    params.addParameter("page",efUserpage);
                    params.addBodyParameter("document",document25.getId());
                    x.http().post(params, new Callback.CommonCallback<String>() {
                        @Override
                        public void onSuccess(String s) {
    ...
    private void toComit(final Boolean submit) {
        if (efUsers == null||efUsers.size()<1){
            ...
            document25.setNo(no.getText().toString());
            document25.setNotice(notice.getText().toString());
            document25.setFddbr(fddbr.getText().toString());
            document25.setFddbrzw(fddbrzw.getText().toString());
            document25.setFrlxdh(frlxdh.getText().toString());
            document25.setCorpName(corpName.getText().toString());
            document25.setAddress(address.getText().toString());
            document25.setYzbm(yzbm.getText().toString());
            document25.setSfdccf(sfdccf.getText().toString());
            document25.setWfss(wfss.getText().toString());
            document25.setWfgd(wfgd.getText().toString());
            document25.setFlyj(flyj.getText().toString());
            document25.setXzcf(xzcf.getText().toString());
            document25.setMoney(Double.parseDouble(money.getText().toString()));
            document25.setFkssdw(fkssdw.getText().toString());
            document25.setFksszh(fksszh.getText().toString());
            document25.setDsrwtdlr(dsrwtdlr.getText().toString());
            document25.setFyjgsj(fyjgsj.getText().toString());
            document25.setFyjgzf(fyjgzf.getText().toString());
            document25.setSsfy(ssfy.getText().toString());
            new Thread(new Runnable() {
                @Override
                public void run() {
                    form.setDocument25(document25);
                    ...
                    RequestParams params = new RequestParams(Constant.getHOST(mContext)+URLs.SAVEDOM_URL + "/25/appsave.do");
