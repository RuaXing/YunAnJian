...
public class Document29Activity extends MFragmentActivity {
    private static final String TAG = "Document29Activity";
    public static Intent getIntent(Context ctx, EfDocument efDocument,Boolean isDone) {
        Intent intent = new Intent(ctx, Document29Activity.class);
    ...
    @ViewInject(R.id.receiveCorp)
    private EditText receiveCorp;

    private Context mContext;
    private EfDocument efdcoment;
    private EfDocument29 document29 = new EfDocument29();
    ...
    protected void onCreate(Bundle savedInstanceState) {
        ...
        setContentView(R.layout.activity_document29);
        x.view().inject(this);
        mContext = Document29Activity.this;
        ...
        domtt.setTitle2(DocumentType.d29.name);
    ...
    private void getData() {
        if (null != efdcoment){
            new Thread(new Runnable() {
                @Override
                public void run() {
                    RequestParams params = new RequestParams(Constant.getHOST(mContext)+URLs.DOCUMENT_URL+"/29/get.do");
                    params.addBodyParameter("id",efdcoment.getDocument());
                    x.http().post(params, new Callback.CommonCallback<String>() {
                        @Override
                        public void onSuccess(String s) {
                            document29 = getGson().fromJson(s,EfDocument29.class);
    ...
    private void initData() {
        no.setText(document29.getNo());
        corp.setText(document29.getCorp());
        startTime.setText(getFmt().format(document29.getStartTime()));
        endTime.setText(getFmt().format(document29.getEndTime()));
        fromNo.setText(document29.getFromNo());
        receiveCorp.setText(document29.getReceiveCorp());
    }
    private void toComit(final Boolean submit) {
        document29.setNo(no.getText().toString());
        document29.setCorp(corp.getText().toString());
        document29.setFromNo(fromNo.getText().toString());
        document29.setReceiveCorp(receiveCorp.getText().toString());
        try {
            document29.setStartTime(getFmt().parse(startTime.getText().toString()));
            document29.setEndTime(getFmt().parse(endTime.getText().toString()));
        ...
        new Thread(new Runnable() {
            ...
            public void run() {
                ...
                RequestParams params = new RequestParams(Constant.getHOST(mContext)+URLs.SAVEDOM_URL+"/29/appsave.do");
