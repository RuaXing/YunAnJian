...
public class InsDocnotdoActivity extends BaseActivity {
    private static final String TAG = "InsDocnotdoActivity";
    public static Intent getIntent(Context ctx, MEfInspect mEfInspect) {
        Intent intent = new Intent(ctx, InsDocnotdoActivity.class);
        intent.putExtra("mEfInspect", mEfInspect);
        return intent;
    }
    ...
    protected void onCreate(Bundle savedInstanceState) {
        ...
        x.view().inject(this);
        mContext = InsDocnotdoActivity.this;
        mEfInspect = (MEfInspect) getIntent().getSerializableExtra("mEfInspect");
        getDocListN();
        getCorp();
    }
    private void getDocListN() {
        new Thread(new Runnable() {
            ...
            public void run() {
                ...
                params.addBodyParameter("inspectId",mEfInspect.getId());
    ...
    private void toItemClink(EfDocumentPageItem obj) {
        ...
        if (DocumentType.d1.name.equals(docName)){
            ...
            startActivityForResult(Document02Activity.getIntent(mContext,corpInfo,docId,caseinfo,inspectId),navId);
